...
export type ProcessedWorkflowAmountStageOverrideLabels = Record<string, Record<string, string>> // configId -> language -> label
...
export class WorkflowAmountStageOverrideLabelsService {
  static async fetchLabels(): Promise<WorkflowAmountStageOverrideLabelResponse[]> {
    try {
      ...
      const labels = await apiClient.get<WorkflowAmountStageOverrideLabelResponse[]>(
  ...
  static processLabels(labels: WorkflowAmountStageOverrideLabelResponse[]): ProcessedWorkflowAmountStageOverrideLabels {
  ...
  static hasLabels(labels: ProcessedWorkflowAmountStageOverrideLabels): boolean {
    return labels && Object.keys(labels).length > 0
  }

  static getAvailableLanguages(labels: ProcessedWorkflowAmountStageOverrideLabels): string[] {
    try {
