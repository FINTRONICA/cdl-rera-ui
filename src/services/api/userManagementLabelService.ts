...
/**
 * Service for managing user management label configurations from the API
 * Handles fetching, caching, and processing of label data
 */
export class UserManagementLabelService {
  ...
  /**
   * Fetch all user management labels from the API
   * @returns Promise<LabelConfig[]>
   */
  async getUserManagementLabels(): Promise<LabelConfig[]> {
    try {
      const url = buildApiUrl(
        API_ENDPOINTS.APP_LANGUAGE_TRANSLATION.USER_MANAGEMENT_LABEL
      )
   ...
   * @returns Promise<LabelConfig[]>
   */
  async getUserManagementLabelsWithCache(): Promise<LabelConfig[]> {
    const now = Date.now()
    const cacheKey = 'user-management-labels'
    ...
    try {
      const labels = await this.getUserManagementLabels()
  ...
  getLabel(configId: string, languageCode: string = 'EN'): string {
    try {
      const cachedData = this.labelCache.get('user-management-labels')
      ...
      const result = matchingLabel?.configValue || getUserManagementLabel(configId)
      return result
    } catch (error) {
      return getUserManagementLabel(configId)
    }
  }
  ...
  async refreshCache(): Promise<void> {
    ...
    await this.getUserManagementLabelsWithCache()
  }
...
export const userManagementLabelService = new UserManagementLabelService()
