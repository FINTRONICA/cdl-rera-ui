...
const UserManagementPage: React.FC = () => {
  ...
  const userManagementTitle = getLabelResolver
    ? getLabelResolver('user', 'Stakeholder')
    : 'Stakeholder'
  ...
  // Hook for API data - fetch all users (large size to get all data)
  const { data, isLoading, error } = useAuthAdminUsers(0, 10000)
  const allUserData = data?.content || []

  const statusOptions = ['ACTIVE', 'CLOSED']

  // Table state hook for search, filtering, and pagination
  const {
    search,
    selectedRows,
    expandedRows,
    ...
    handleRowsPerPageChange: handleTableRowsPerPageChange,
    sortConfig,
    handleSort,
  } = useTableState({
    data: allUserData,
  ...
  // Handle page changes (convert 1-based to 0-based for display)
  ...
  const handleRowsPerPageChange = (newRowsPerPage: number) => {
    handleTableRowsPerPageChange(newRowsPerPage)
  }
        ...
        <div className="bg-white/75 dark:bg-gray-800/80 rounded-2xl flex flex-col h-full">
          ...
          <div className="sticky top-0 z-10 bg-white/75 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700 rounded-t-2xl">
            ...
            {selectedRows.length > 0 && (
              <div className="flex justify-end gap-2 py-3.5 px-4 border-b border-gray-200 dark:border-gray-700">
                {actionButtons.map((button, index) => (
                  <button
                    key={index}
                    onClick={button.onClick}
                    disabled={button.disabled}
                    className={`flex items-center h-8 py-1.5 px-2.5 gap-1.5 font-medium text-sm rounded-md ${button.variant === 'primary' ? 'bg-blue-600 dark:bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200'} ${button.disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    {button.icon && (
