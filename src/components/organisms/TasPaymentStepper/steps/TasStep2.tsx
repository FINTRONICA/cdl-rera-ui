...
const TasStep2: React.FC = () => {
  ...
  const Section = ({ title, fields, isDarkMode }: SectionProps) => (
    <Box mb={4}>
      <Typography
        variant="h6"
        fontWeight={600}
  ...
  if (loading) {
    return (
      <Box
        ...
        minHeight="400px"
        sx={{
          backgroundColor: isDarkMode ? '#1E293B' : 'rgba(255, 255, 255, 0.75)',
        }}
      >
        <CircularProgress />
        <Typography
          sx={{ ml: 2, color: isDarkMode ? '#CBD5E1' : 'text.secondary' }}
        >
          Loading...
        </Typography>
      </Box>
