...
export const budgetCategoryDocumentService: DocumentService<
  DocumentItem,
  ApiDocumentResponse
> = {
  getDocuments: async (entityId: string, page = 0, size = 20) => {
...
export const mapApiToBudgetCategoryDocumentItem = (
...
export const budgetCategoryDocumentActions: DocumentAction<DocumentItem>[] = [
  {
    key: 'download',
    label: 'Download',
...
export const createBudgetCategoryDocumentConfig = (
  entityId: string,
  options?: {
    title?: string
    description?: string
  ...
  const actions = [...budgetCategoryDocumentActions]
  if (options?.onDelete) {
  ...
  const config: DocumentUploadConfig<DocumentItem, ApiDocumentResponse> = {
    ...
    documentService: budgetCategoryDocumentService,
    mapApiToDocument: mapApiToBudgetCategoryDocumentItem,
    documentTypeSettingKey: 'INVESTOR_ID_TYPE', // Using INVESTOR_ID_TYPE as document types are shared (same as Management Firm)
    title: options?.title || 'Budget Category Documents',
    description:
      options?.description ||
