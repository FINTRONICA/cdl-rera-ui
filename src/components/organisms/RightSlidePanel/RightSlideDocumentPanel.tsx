...
import { Drawer, Box, DialogTitle, DialogContent, IconButton, useTheme } from "@mui/material";
      ...
      >
        ...
        >
          ...
          >
            ...
            <p className="text-slate-500 dark:text-slate-400">Drag & Drop Your File Here</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
              Please upload PDF, Excel, or Word Documents up to 25 MB
            </p>
            <label className="cursor-pointer">
              <div className="bg-blue-600 dark:bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-700 dark:hover:bg-blue-400 transition text-sm w-40 mx-auto">
                Browse Files
              </div>
          ...
          <div className="space-y-3 overflow-y-auto w-94">
            {files.map((file) => (
              <div key={file.id} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm bg-white dark:bg-gray-800">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="">
                      <p className="font-medium text-sm text-slate-800 dark:text-slate-200 max-w-[250px]">
                        {file.name}
                      </p>
                  ...
                  {/* Right: Status/Cancel */}
                  {file.status === "completed" ? (
                    <CheckCircle className="text-green-500 dark:text-green-400 w-5 h-5 flex-shrink-0 mt-2" />
                  ) : (
                    <CancelOutlinedIcon
                      className="cursor-pointer text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 flex-shrink-0 mt-2"
                      onClick={() =>
                ...
                <div className="mt-3 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div
                    className="h-2 rounded-full transition-all bg-blue-600 dark:bg-blue-500"
                    style={{ width: `${file.progress}%` }}
                  />
                </div>
                <p className="text-right text-xs font-medium text-gray-600 dark:text-gray-400 mt-1">
                  {file.progress}%
                </p>
              </div>
            ))}
